...
# 🔑 Carrega segredos (chaves de API usadas nas URLs de wg-blacklist.conf)
if [[ -f "$ENV_FILE" ]]; then
    set -a
    source "$ENV_FILE"
    set +a
else
    echo "⚠️  ${ENV_FILE} não encontrado. Listas que dependem de chave serão puladas."
fi
...
while IFS=";" read -r LIST_NAME URL; do
    ...
    # Expande placeholders ${VAR} da URL com as variáveis vindas do .env
    SKIP_LIST=0
    while [[ "$URL" =~ \$\{([A-Za-z_][A-Za-z0-9_]*)\} ]]; do
        VAR_NAME="${BASH_REMATCH[1]}"
        VAR_VALUE="${!VAR_NAME:-}"
        if [[ -z "$VAR_VALUE" ]]; then
            echo "⚠️  ${LIST_NAME}: variável ${VAR_NAME} não definida no .env. Pulando esta lista."
            SKIP_LIST=1
            break
        fi
        URL="${URL//\$\{${VAR_NAME}\}/${VAR_VALUE}}"
    done
    [[ "$SKIP_LIST" -eq 1 ]] && continue
